...
The verify-then-reason-then-ground sequence itself lives in desk/pipeline.py
(Phase 5), not here. This file's run_case() is a corpus adapter: it loads a case's
on-disk artifacts (context.json, narrative.json, saml_response.xml), decides whether
there is a SAMLResponse to verify at all, calls desk.pipeline.run_pipeline() to do
the actual work, and then layers corpus-only concerns on top of the result --
stored-check-result parity against corpus/MANIFEST.json, and CaseRecord/JobRecord
serialization for records.json. Any future live case orchestrator calls
run_pipeline() directly and never needs this file, matching the layering direction
eval/metrics.py already established (eval/ depends on desk/, never the reverse).
...
and harness/generate.py by direct inspection, not assumed):
  ...
  directly (resolved.raw is None there). run_case() passes raw_saml_response=None into
  desk.pipeline.run_pipeline() to mirror that exactly, rather than synthesizing an
  empty-bytes parse error, which looks similar but is caused differently and was
  rejected during Phase 4 smoke-testing for exactly that reason.
    ...
    *,
    ...
    if label.get("no_saml_response_reason"):
        ...
        if not saml_path.exists():
            # Every other case in the corpus carries a saml_response.xml. A case
            # missing one without no_saml_response_reason set is a harness bug, not
            # something to skip quietly.
            return CaseRecord(
                case_id=case_id,
                label=label,
                verify_state="error",
                verify_error=f"{case_id}: no saml_response.xml on disk and no_saml_response_reason unset",
                check_counts=None,
                check_results=None,
        ...
        raw_saml_response = saml_path.read_bytes()
    ...
    check_counts = None
    check_results = None
    if result.verify_state in ("ok", "parse_error") and result.run is not None:
        stored_parity_ok = check_stored_parity(result.run, stored)
        check_results = _check_results_to_json(result.run)
        if result.verify_state == "ok":
            check_counts = result.run.counts()
    elif result.verify_state == "no_saml_response":
    ...
    return CaseRecord(
        ...
        job_a=JobRecord.from_job_result(result.job_a) if result.job_a else None,
        job_b=JobRecord.from_job_result(result.job_b) if result.job_b else None,
        job_c=JobRecord.from_job_result(result.job_c) if result.job_c else None,
        grounding=GroundingRecord.from_result(result.grounding) if result.grounding else None,
